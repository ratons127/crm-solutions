'use client';
import Breadcrumbs from '@/components/common/Breadcrumbs';
import StatusField from '@/components/common/StatusField';
import { DataTable } from '@/components/common/table/DataTable';
import RowActionDropdown from '@/components/common/table/RowActionDropdown';
import {
  useDeleteShiftCategoryMutation,
  useGetPaginatedShiftCategoriesQuery,
} from '@/lib/features/shiftCategories/shiftCategoriesAPI';
import { ShiftCategory } from '@/lib/types/shiftCategories';
import { PencilSquareIcon, TrashIcon } from '@heroicons/react/24/outline';
import { Modal, Paper, Text } from '@mantine/core';
import { modals } from '@mantine/modals';
import { notifications } from '@mantine/notifications';
import { ColumnDef } from '@tanstack/react-table';
import { useMemo, useState } from 'react';
import { BsThreeDots } from 'react-icons/bs';
import ShiftCategoriesForm from './ShiftCategoriesForm';

const ShiftCategoriesPage = () => {
  const [action, setAction] = useState<null | 'create' | 'update'>(null);
  const [item, setItem] = useState<ShiftCategory | null>(null);
  const [deleteItem] = useDeleteShiftCategoryMutation();

  // Server-side pagination state
  const [pageIndex, setPageIndex] = useState(0); // 0-based for DataTable
  const [pageSize, setPageSize] = useState(10);

  // Fetch paginated data
  const { data, isLoading } = useGetPaginatedShiftCategoriesQuery({
    page: pageIndex + 1, // API uses 1-based page numbers
    perPage: pageSize,
    sortDirection: 'DESC',
  });

  const columns: ColumnDef<ShiftCategory, ShiftCategory>[] = useMemo(
    () => [
      {
        id: 'select',
        enableSorting: false,
        enableHiding: false,
        header: ({ table }) => (
          <input
            type="checkbox"
            className="h-4 w-4 accent-blue-600 cursor-pointer"
            checked={table.getIsAllPageRowsSelected()}
            onChange={table.getToggleAllPageRowsSelectedHandler()}
          />
        ),
        cell: ({ row }) => (
          <input
            type="checkbox"
            className="h-4 w-4 accent-blue-600 cursor-pointer"
            checked={row.getIsSelected()}
            disabled={!row.getCanSelect()}
            onChange={row.getToggleSelectedHandler()}
          />
        ),
      },
      {
        accessorKey: 'id',
        header: 'Category ID',
      },
      {
        accessorKey: 'name',
        header: 'Category Name',
      },
      {
        accessorKey: 'type',
        header: 'Type',
        cell: ({ row }) => {
          const type = row.original.type;
          const isRegular = type?.toLowerCase() === 'regular';
          const isNight = type?.toLowerCase() === 'night';

          return (
            <span
              className={`inline-flex items-center px-3 py-1 rounded text-sm font-medium ${
                isRegular
                  ? 'bg-[#DBEAFE] text-[#193CB8]'
                  : isNight
                    ? 'bg-[#F3E8FF] text-[#6E11B0]'
                    : 'bg-[#FFEDD4] text-[#9F2D00]'
              }`}
            >
              {type}
            </span>
          );
        },
      },
      {
        accessorKey: 'description',
        header: 'Description',
        cell: ({ row }) => <span>{row.original.description || 'N/A'}</span>,
      },
      {
        accessorKey: 'status',
        header: 'Status',
        cell: ({ row }) => <StatusField status={row.original.status} />,
      },
      {
        id: 'actions',
        header: 'Actions',
        cell: ({ row }) => (
          <RowActionDropdown
            data={[
              {
                label: 'Edit',
                icon: <PencilSquareIcon height={16} />,
                action: () => {
                  setAction('update');
                  setItem(row.original);
                },
              },
              {
                label: 'Delete',
                icon: <TrashIcon height={16} />,
                action: () => {
                  modals.openConfirmModal({
                    title: 'Delete confirmation',
                    centered: true,
                    children: (
                      <Text size="sm">
                        Are you sure you want to delete ? This action is
                        destructive and you will have to contact support to
                        restore your data.
                      </Text>
                    ),
                    labels: {
                      confirm: 'Delete',
                      cancel: 'Cancel',
                    },
                    size: 'lg',
                    confirmProps: { color: 'red' },
                    onCancel: () => {
                      setAction(null);
                      setItem(null);
                    },
                    onConfirm: async () => {
                      try {
                        await deleteItem({ id: row.original.id });
                        notifications.show({
                          title: `Shift Category Deleted successfully`,
                          message: '',
                          color: 'red',
                        });
                      } catch (error) {
                        notifications.show({
                          title: 'Failed to delete',
                          message: 'Something went wrong',
                        });
                      }
                    },
                  });
                },
              },
            ]}
          >
            <BsThreeDots />
          </RowActionDropdown>
        ),
      },
    ],
    [deleteItem]
  );

  // Generate dynamic filter options from data
  const uniqueTypes = Array.from(
    new Set(data?.data?.map((cat: ShiftCategory) => cat.type).filter(Boolean))
  ).map(type => ({
    label: String(type),
    value: String(type),
  }));

  const selectFilters: any = [
    {
      id: 'type',
      label: 'Type',
      options: uniqueTypes,
    },
    {
      id: 'status',
      label: 'Status',
      options: [
        { label: 'Enabled', value: true },
        { label: 'Disabled', value: false },
      ],
    },
  ];

  return (
    <>
      {/* <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        <WorkCard
          icon={<PiStack className="h-5 w-5" />}
          label="Total Categories"
          value="4"
          bgGradient="bg-gradient-to-r from-[#EFF6FF] to-[#DBEAFE]"
          labelClass="text-[#155DFC]"
          valueClass="text-[#193CB8]"
          iconBg="bg-[#2B7FFF]"
          borderClass="border border-[#BEDBFF]"
        />
        <WorkCard
          icon={<SiTicktick className="h-5 w-5" />}
          label="Active Categories"
          value="3"
          bgGradient="bg-gradient-to-r from-[#F0FDF4] to-[#DCFCE7]"
          labelClass="text-[#00A63E]"
          valueClass="text-[#016630]"
          iconBg="bg-[#00C950]"
          borderClass="border border-[#B9F8CF]"
        />
        <WorkCard
          icon={<IoCloseCircleOutline className="h-5 w-5" />}
          label="Inactive Categories"
          value="1"
          bgGradient="bg-gradient-to-r from-[#FFF7ED] to-[#FFEDD4]"
          labelClass="text-[#F54900]"
          valueClass="text-[#9F2D00]"
          iconBg="bg-[#FF6900]"
          borderClass="border border-[#FFD6A7]"
        />
        <WorkCard
          icon={<PiStack className="h-5 w-5" />}
          label="Total Employees"
          value="340"
          bgGradient="bg-gradient-to-r from-[#FAF5FF] to-[#F3E8FF]"
          labelClass="text-[#9810FA]"
          valueClass="text-[#6E11B0]"
          titleClass="text-white/70"
          iconBg="bg-[#AD46FF]"
          borderClass="border border-[#E9D4FF]"
        />
      </div> */}
      <Paper p={20} radius={5} shadow="sm">
        <DataTable<ShiftCategory>
          data={data?.data ?? []}
          columns={columns}
          selectFilters={selectFilters}
          searchPlaceholder="Search shift categories..."
          csvFileName="Shift Categories"
          enableRowSelection
          onPressAdd={() => {
            setAction('create');
          }}
          manualPagination
          pageCount={data?.meta?.lastPage ?? 0}
          pagination={{ pageIndex, pageSize }}
          onPaginationChange={({ pageIndex, pageSize }) => {
            setPageIndex(pageIndex);
            setPageSize(pageSize);
          }}
          totalRows={data?.meta?.total}
          loading={isLoading}
          headerLeft={<Breadcrumbs />}
        />
      </Paper>
      {/* Modal */}
      <Modal
        opened={action !== null}
        onClose={() => {
          setAction(null);
        }}
      >
        <ShiftCategoriesForm
          action={action}
          onClose={() => setAction(null)}
          data={item}
        />
      </Modal>
    </>
  );
};

export default ShiftCategoriesPage;
