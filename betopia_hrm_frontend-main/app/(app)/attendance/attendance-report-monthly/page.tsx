...
export default function AttendanceReportMonthly() {
  ...
  // Daily report
  const [getMonthlyReport, { isLoading: fetchingReport }] =
    useGetAttendanceMonthlyReportMutation();
  const [rows, setRows] = useState<any[]>([]);

  // Export
  const [exportReport] = useExportAttendanceMonthlyReportMutation();
  ...
  const handleExport = async (format: 'excel' | 'pdf' | 'csv') => {
    ...
    const fname = `attendance_monthly_${fromDate ?? 'from'}_${toDate ?? 'to'}.${ext}`;
    try {
  ...
  const onApplyFilters = async () => {
    ...
    try {
      const res = await getMonthlyReport(payload).unwrap();
      setRows(res?.data ?? []);
    } catch {
      setRows([]);
    }
  ...
  const columns: ColumnDef<any, any>[] = useMemo(() => {
    const Pill = (value: number | null | undefined, color: string) => (
      <span
        className={`min-w-8 inline-flex items-center justify-center px-2 py-1 rounded-full text-xs font-semibold ${color}`}
      >
        {value ?? 0}
      </span>
    );
    ...
    return [
      ...
      },
      {
        ...
        cell: ({ getValue }) =>
          Pill(Number(getValue() ?? 0), 'bg-slate-100 text-slate-700'),
      },
      {
        header: 'Present Days',
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            {Pill(row.original.presentDays, 'bg-green-100 text-green-700')}
            <span className="text-xs text-green-700">
              (
              {pct(
                Number(row.original.presentDays ?? 0),
                Number(row.original.totalWorkingDays ?? 0)
              )}
              %)
            </span>
          </div>
      ...
      },
      {
        ...
        header: 'Absent Days',
        cell: ({ row }) =>
          Pill(row.original.absentDays, 'bg-red-100 text-red-700'),
      },
      {
        accessorKey: 'leaveDays',
        header: 'Leave',
        cell: ({ row }) =>
          Pill(row.original.leaveDays, 'bg-indigo-100 text-indigo-700'),
      },
      {
        accessorKey: 'lateDays',
        header: 'Late Days',
        cell: ({ row }) =>
          Pill(row.original.lateDays, 'bg-amber-100 text-amber-700'),
      },
      {
        accessorKey: 'earlyLeaveDays',
        header: 'Early Out Days',
        cell: ({ row }) =>
          Pill(row.original.earlyLeaveDays, 'bg-orange-100 text-orange-700'),
      },
      {
        accessorKey: 'manualAttendance',
        header: 'Manual Attendance',
        cell: ({ row }) =>
          Pill(row.original.manualAttendance, 'bg-sky-100 text-sky-700'),
      },
      {
        accessorKey: 'offDays',
        header: 'Off Days',
        cell: ({ row }) =>
          Pill(row.original.offDays, 'bg-purple-100 text-purple-700'),
      },
      {
        accessorKey: 'holidays',
        header: 'Holidays',
        cell: ({ row }) =>
          Pill(row.original.holidays, 'bg-blue-100 text-blue-700'),
      },
