...
import { EmploymentTypesData } from '@/lib/types/employment';
...
import EmploymentTypesForm from './EmploymentTypesForm';

export default function EmploymentType() {
  const [action, setAction] = useState<null | 'create' | 'update'>(null);
  const [item, setItem] = useState<EmploymentTypesData | null>(null);
  ...
  const { data, isLoading } = useGetPaginatedEmploymentTypesQuery({
    page: pageIndex + 1, // API uses 1-based page numbers
    perPage: pageSize,
    sortDirection: 'DESC',
  });

  const columns: ColumnDef<EmploymentTypesData, EmploymentTypesData>[] =
    useMemo(
      () => [
        // {
        //   id: 'select',
                    ...
                    modals.openConfirmModal({
                      ...
                      onConfirm: async () => {
                        try {
                          ...
                          notifications.show({
                            title: `Employment Types Deleted successfully`,
                            message: '',
                            color: 'red',
                          });
                        } catch (error) {
  ...
  return (
    <div>
      <Paper p={20} radius={5} shadow="sm">
        <Breadcrumbs />
        <DataTable<EmploymentTypesData>
          data={data?.data ?? []}
          columns={columns}
          selectFilters={[]}
          searchPlaceholder="Search..."
          csvFileName="Employment Types"
          enableRowSelection
          hideExport
          onPressAdd={() => {
            setAction('create');
