...
import { InstituteNameData } from '@/lib/types/qualification';
...
import InstituteNameForm from './InstituteNameForm';

export default function InstituteName() {
  const [action, setAction] = useState<null | 'create' | 'update'>(null);
  const [item, setItem] = useState<InstituteNameData | null>(null);
  const [deleteItem] = useDeleteInstituteNameMutation();
  ...
  const { data, isLoading } = useGetPaginatedInstituteNamesQuery({
    page: pageIndex + 1, // API uses 1-based page numbers
    perPage: pageSize,
    sortDirection: 'DESC',
  });

  const columns: ColumnDef<InstituteNameData, InstituteNameData>[] = useMemo(
    () => [
      // {
      //   id: 'select',
      //   enableSorting: false,
  ...
  return (
    <div>
      <Paper p={20} radius={5} shadow="sm">
        <Breadcrumbs />
        <DataTable<InstituteNameData>
          data={data?.data ?? []}
          columns={columns}
          selectFilters={[]}
          searchPlaceholder="Search..."
          csvFileName="Institute Name"
          enableRowSelection
          hideExport
          onPressAdd={() => {
            setAction('create');
      ...
      {/* Modal */}
      <Modal
        title={`Institute Name ${action === 'create' ? 'Create' : 'Update'}`}
        opened={action !== null}
        onClose={() => {
          setAction(null);
        }}
