...
import { EmployeeQualificationType } from '@/lib/types/qualification';
...
export default function QualificationType() {
  ...
  const [item, setItem] = useState<EmployeeQualificationType | null>(null);
  const [deleteItem] = useDeleteQualificationTypeMutation();
  ...
  const { data, isLoading } = useGetPaginatedQualificationTypesQuery({
  ...
  return (
    <div>
      <Paper p={20} radius={5} shadow="sm">
        <Breadcrumbs />
        <DataTable<EmployeeQualificationType>
          data={data?.data ?? []}
          columns={columns}
          selectFilters={[]}
          searchPlaceholder="Search..."
          csvFileName="Employee Qualification type"
          enableRowSelection
          hideExport
          onPressAdd={() => {
            setAction('create');
      ...
      {/* Modal */}
      <Modal
        title={`Qualification Type ${action === 'create' ? 'Create' : 'Update'}`}
        opened={action !== null}
        onClose={() => {
          setAction(null);
        }}
